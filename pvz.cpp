...
#define ELEMENT_HEIGHT 140
#define SOUND_FILE "./sound/RigorMormist.mp3"
#define SOUND_FILE1 "./sound/zombie_eating1.wav"
#define BACK_BUTTON "./Image_Assets/button_middle.jpg"
#define ZOMBIE_DIRETORY2 "./Image_Assets/zombie_cached_4.png"
#define SEED_DIRECTORY "./Image_Assets/SeedBankCoop.png"
...
#define ZOMBIE_HEALTHY_DIRECTORY "./Image_Assets/zombie_cached_0.png"
...
#define PEA_DIRECTORY "./Image_Assets/ProjectilePea.png"
...
void determine_icon_chosen(Icons & icons, int mouse_y,int mouse_x);
...
bool user_clicked_on_back(int mouse_x, int mouse_y);
...
void determine_zombie_appearanc(Zombie & zombie){
	if (zombie.health > 10)
		zombie.directory = ZOMBIE_DIRETORY2;
	else zombie.directory = ZOMBIE_HEALTHY_DIRECTORY;
...
void create_new_zombies(Elements & elements, Level & level){
	...
	if (level.waves_finished == false){
		...
		for (int i = 0; i < zombie_cnt; i++){
			if (radom == 2) {
				temp.health = 40;
				temp.directory = ZOMBIE_DIRETORY2;
			}
...
void display_game_layout(window & win, Player player, Icons icons){
	...
	win.draw_png(BACK_BUTTON, 900, 5, 100, 40);
	win.show_text("BACK", 920, 8);
	win.draw_png(SEED_DIRECTORY,100,5,500,85);
	win.draw_png(PEASHOOTER_100_ICON_BRIGHT_DIRECTORY, 220, 13, 60, 70);
	win.draw_png(PEASHOOTER_DIRECTORY, 220, 5, 60, 75);
	win.show_text("100", 220, 60);
	win.show_text("50", 340, 60);
	win.draw_png(PEASHOOTER_100_ICON_BRIGHT_DIRECTORY, 280, 13, 60, 70);
	win.show_text("50", 285, 60);
	win.draw_png(SUNFLOWER_DIRECTORY, 280, 5, 60, 75);
	win.draw_png(PEASHOOTER_100_ICON_BRIGHT_DIRECTORY, 340, 13, 60, 70);
	win.show_text("50", 345, 60);
	win.draw_png(WALNUT_UNTOUCHED_DIRECTORY, 340, 5, 60, 75);
	win.show_text(to_string(player.sun_count), 140, 60);
...
void display_zombies(window & win, vector<Zombie> & zombies, Map & map){
	for (int i = 0; i < zombies.size(); i++){
		...
		win.draw_png(zombies[i].directory, zombies[i].x_location, y_location-35, 140, 180);
	}
}
...
void display_peashooters(window & win, vector<Peashooter> & peashooters, Map & map){
	for (int i = 0; i < peashooters.size(); i++){
		...
		win.draw_png(PEASHOOTER_DIRECTORY, map[row][col].x1 -10, map[row][col].y1 , ELEMENT_WIDTH, ELEMENT_HEIGHT);
	}
}
...
void display_peas(window & win, vector<Pea> & peas, Map & map){
	for (int i = 0; i < peas.size(); i++){
		...
		if (peas[i].x_location < (map[0][8].x2 - 35))	
			win.draw_png(PEA_DIRECTORY, peas[i].x_location, y_location+20, PEA_WIDTH, PEA_HEIGHT);
	}
}
...
void display_sunflowers(window & win, vector<Sunflower> & sunflowers, Map & map){
	for (int i = 0; i < sunflowers.size(); i++){
		...
		win.draw_png(SUNFLOWER_DIRECTORY, map[row][col].x1-10, map[row][col].y1, ELEMENT_WIDTH, ELEMENT_HEIGHT);
	}
}
...
void display_walnuts(window & win, vector<Walnut> & walnuts, Map & map){
	for (int i = 0; i < walnuts.size(); i++){
		...
		win.draw_png(walnuts[i].directory, map[row][col].x1-20, map[row][col].y1, ELEMENT_WIDTH+20, ELEMENT_HEIGHT);
	}
}
...
void handle_user_click(Player & player, Icons & icons, Elements & elements, Map & map, int mouse_x, int mouse_y){
	...
	if (player.is_first_click_made == false){
		if (is_an_icon_chosen(mouse_x, mouse_y)){
			determine_icon_chosen(icons, mouse_y,mouse_x);
...
bool is_an_icon_chosen(int mouse_x, int mouse_y){
	if (mouse_x > 100 && mouse_x < 600 &&
		mouse_y > 5 && mouse_y < 90)
		return true;
	return false;
}
...
bool user_clicked_on_back(int mouse_x, int mouse_y) {
	if (mouse_x > 900 && mouse_x < 1000  &&
		mouse_y > 5 && mouse_y < 45)
		return true;
	return false;
}
bool user_clicked_on_start(int mouse_x, int mouse_y){
...
int main(int argc ,char* argv[]) {
	...
	Mix_Music* music1 = Mix_LoadMUS(SOUND_FILE);
	...
	while(!quit){
		...
		HANDLE(
			...
			KEY_PRESS(q, quit = true);
			LCLICK({
				if (user_clicked_on_back(mouse_x,mouse_y)) desplay_starting_screen(win);
